...
pathway=Reader.get_UpperPath('.')+'/Gait-Tracking-With-x-IMU-Python-master/Roy6BA93_9axis_220609rollpitchyaw90.csv'  ##小板 記得改mahony rate
...
# pathway=Reader.get_UpperPath('.')+'/Gait-Tracking-With-x-IMU-Python-master/Roy62E24_9axis_220617static.csv'  ##大阪 記得改mahony rate
...
# gyrX=raw[:,4]-np.mean(raw[:,4])
# gyrY=raw[:,5]-np.mean(raw[:,5])
# gyrZ=raw[:,6]-np.mean(raw[:,6])
magX=raw[:,7]
magY=raw[:,8]
magZ=raw[:,9]
plt.title('mag')
...
pathway=Reader.get_UpperPath('.')+'/Gait-Tracking-With-x-IMU-Python-master/Roy6BA93_quater_220609rollpitchyaw90.csv' 
...
mahony = ahrs.filters.Mahony(Kp=1, Ki=0, frequency=1/mahonyrate)
if useMahony:
    ...
    for t in range(0,ts.size):
        ...
        # gyr = np.array([gyrY[t],-gyrX[t],gyrZ[t]])*np.pi/180#default
        # acc = np.array([accY[t],-accX[t],accZ[t]])#default
        
        # mag = np.array([magX[t],-magY[t],-magZ[t]])
        mag = np.array([magX[t],-magY[t],-magZ[t]])
        # mag = np.array([-magY[t],magX[t],magZ[t]])
        # mag = np.array([-magY[t],-magX[t],-magZ[t]]) #default
...
if useMadgwick:
    ...
    quat,eu=calc_orientation_mag(gyr/180*np.pi,acc,mag,1/samplePeriod,align_arr,beta,delt_t/4,4)
