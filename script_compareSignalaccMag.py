...
def main():
    ...
    velprocess=False
    # velprocess=True
    threshold=0.05  #unity's setting value is 0.03, probably 0.2-0.3 is better
    #----------------------------------------------------
    ...
    pathway=Reader.get_UpperPath('.')+'/Gait-Tracking-With-x-IMU-Python-master/203103536_positionCalculator.csv'
    ...
    ax1.plot(raw[:,10:11],linewidth=0.5)
    ax1.plot(pos[:,0],linewidth=0.5)
    ax1.set_title("pos_x")
    # ax1.set_xlabel("n")
    ax1.set_ylabel("position_x (m)")
    ax2.plot(raw[:,11:12],linewidth=0.5)
    ax2.plot(newAcc[:,2],linewidth=0.5)
    ax2.set_title("pos_y")
    # ax2.set_xlabel("n")
    ax2.set_ylabel("position_y (m)")
    ax3.plot(raw[:,12:13],linewidth=0.5)
    ax3.plot(newAcc[:,1],linewidth=0.5)
    ax3.set_title("pos_z")
    ax3.set_xlabel("n")
