...
def main():
    ...
    # export quaternion to csv
    # alldata=np.ones([len(accX),7])
    # alldata[:,0]=accX
    # alldata[:,1]=accY
    # alldata[:,2]=accZ
    # alldata[:,3]=quat[:,0]
    ...
    fig = plt.figure(figsize=(10, 5))
    plt.plot(time,velDrift[:,0],c='r',linewidth=0.5)
    plt.plot(time,velDrift[:,1],c='g',linewidth=0.5)
    plt.plot(time,velDrift[:,2],c='b',linewidth=0.5)
